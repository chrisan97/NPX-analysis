function plotOptoStimPowerLevels(sp, goodClusters, visStimTimes, myKsDir, mw_Table)
%
...
for i = 1:length(goodClusters) % for all the good clusters in my recording
    ...
    for plvl = 1:length(powerLevels)
        ...
        firstAfterStimIdx = find(edges > 0.5,1); % Truncate after 500ms)
        tempPeaksPSTH = spikeRatePSTH(firstAfterStimIdx:end);
        [pks,locs] = findpeaks(tempPeaksPSTH); % only look for peaks in this window
        % scatter(locs+firstAfterStimIdx-1,pks,'r','filled','HandleVisibility','off');
        % hold on;
        peakLatency = [peakLatency diff(locs)*binSize];
    end
    ...
    for plvl = 1:length(powerLevels)
        ...
        plot(spikeRatePSTH,'Color',[0 0 0 plvl*0.17],'LineWidth',1.7)
        hold on;
    end
